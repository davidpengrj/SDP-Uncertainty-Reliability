...
通过 Spearman 相关性分析 (`analysis_ranking_MCC.png`, `analysis_ranking_AUC.png`)，我们发现：
...
从 Trade-off 散点图 (`analysis_tradeoff_auc_ece.jpg`) 来看：
...
漂移路径图 (`analysis_robustness_arrows.jpg`) 显示：
...
├── benchmark_results_IVDP.csv # 生成的 IVDP 原始数据
├── benchmark_results_CVDP.csv # 生成的 CVDP 原始数据
├── analysis_ranking_MCC.png   # 实验结果图：不确定性与 MCC 的相关性排行
├── analysis_tradeoff_auc_ece.jpg # 实验结果图：AUC vs ECE
└── README.md                  # 说明文档
```
...
```bash
pip install pandas numpy scikit-learn matplotlib seaborn torch imbalanced-learn joblib
```
